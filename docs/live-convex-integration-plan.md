...
**Status:** Implementation merged; live deployment verification pending
**Date:** 2026-09-15
**Repository:** `tapit`
...
Links, Cards, Analytics, and Admin operations are now wired to Convex. Their live-browser evidence and production cutover gates remain open.
...
- `convex/schema.ts` already models customers, profiles, links, cards, analytics, audit logs, invitations, deletion requests, and settings.
...
- A non-production deployment has not yet been exercised by the live browser suite; deployment configuration, supported Password-account provisioning, and live E2E execution remain outstanding.
...
The code and local test harness for the live application boundary are merged in `main` at `bb5fab6`. The current local evidence is `npm run verify` (71 tests and a production build), `npm run test:e2e:demo -- --workers=1` (14 tests), `npx convex ai-files status` (all managed guidance current), and `git diff --check`.

The remaining acceptance work is operational: select and record a non-production Convex deployment, provision the two supported Password identities, run `npm run test:e2e:live`, then perform the staging cutover and manual device/public-profile checks.
...
**Status:** Local baseline complete; deployment target setup remains open.

- [ ] Confirm the authenticated Convex CLI account and the selected development deployment.
- [ ] Replace placeholder Convex values in local environment files only; keep `.env.example` safe and descriptive.
- [ ] Run Convex code generation/typechecking against the selected development deployment.
- [ ] Record the deployment target without exposing credentials in source control or logs.
- [ ] Add or update a documented command for starting Next.js and Convex together during live development.
...
**Exit gate:** the repository and demo regression surface are green. The selected development deployment and live configuration are still pending.
...
**Status:** Implementation complete; live browser session proof remains open.
...
- [ ] Prove that a browser login creates a real Convex Auth session and that sign-out invalidates it.

**Exit gate:** provider, proxy, route guards, and loading states are implemented. A fresh live browser session still needs to be proven.
...
**Status:** Implementation and local authorization coverage complete; live account provisioning remains open.
...
- [ ] Decide and document how the seeded administrator first signs in without exposing an open administrator signup path.
- [x] Remove demo password/session calls from the live path while retaining them only behind the explicit demo mode boundary.

**Exit gate:** the live auth flows and negative authorization behavior are implemented and covered locally. Seeded live-account login, setup, and session refresh remain to be exercised.
...
**Status:** Implementation and local ownership/publication coverage complete; live browser proof remains open.
...
- [x] Ensure a draft bio, draft links, or draft image never appear publicly before publication.
...
**Exit gate:** code-level ownership, draft/publication, and public-projection behavior is covered locally. The same journey must still be run against live Convex.
...
**Status:** Bootstrap, integration coverage, and live-test harness implemented; controlled deployment execution remains open.
...
**Exit gate:** local integration tests prove positive and negative authorization cases and bootstrap repeatability. The live browser test has not yet completed against a deployment.
...
**Status:** Convex backend/UI migration and local coverage complete; live execution evidence remains open.
...
**Exit gate:** live-mode code paths and local authorization coverage are complete, with live browser execution still pending.

### Phase 6 — Cutover and cleanup

**Status:** Open; this is the current work phase.

- [ ] Run the complete live verification matrix against a non-production deployment.
- [ ] Set `NEXT_PUBLIC_DEMO_MODE=false` only in the intended live environment and verify the application fails clearly if required Convex configuration is missing.
- [ ] Retire demo-mode links and copy from the live UI while keeping demo mode available for isolated local regression until it is intentionally removed.
- [ ] Update README, `.env.example`, deployment notes, and the production checklist.
- [ ] Run a final authorization review of every public Convex function and route.
- [ ] Confirm no secrets, local tokens, or personal seed credentials are tracked.

**Exit gate:** production-mode build, integration tests, live browser tests, accessibility checks, and a manual public-profile smoke test all pass from a clean checkout.
...
- Links, Cards, Analytics, and Admin operations are migrated in the live-mode code paths and covered locally; their live deployment evidence remains part of the open cutover gate.
